...
Tracking::Tracking(System *pSys, ORBVocabulary *pVoc, FrameDrawer *pFrameDrawer,
    ...
    : mState(NO_IMAGES_YET),
      ...
      time_recently_lost(15.0),
      time_recently_lost_visual(15.0),
      mnInitialFrameId(0),
      mbCreatedMap(false),
      mnFirstFrameId(0),
      mpCamera2(nullptr) {
...
cv::Mat Tracking::GrabImageStereo(const cv::Mat &imRectLeft,
  ...
  // cv::imshow("left", mImGray); cv::waitKey(1);
  // cv::imshow("right", imGrayRight); cv::waitKey(1);
  ...
  if (mState != NO_IMAGES_YET) {
    if (mLastFrame.mTimeStamp > mCurrentFrame.mTimeStamp) {
      cerr
          ...
          << endl;
      cout << "LastFrame: " << mLastFrame.mTimeStamp << endl;
      cout << "CurrentFrame: " << mCurrentFrame.mTimeStamp << endl;
      unique_lock<mutex> lock(mMutexImuQueue);
    ...
    if (bOK)
      ...
      if (mSensor == System::IMU_MONOCULAR || mSensor == System::IMU_STEREO) {
        Verbose::PrintMess("Track lost for less than one second...",
                           Verbose::VERBOSITY_NORMAL);
        // JL_EDIT: When we lost track before third initilization, no need to
        // reset the map
        if (!pCurrentMap->isImuInitialized() ||
        ...
        // cout << "System set to RECENTLY_LOST" << endl;
        mState = RECENTLY_LOST;
      } else
