...
cv::Mat visualize(cv::Mat imGray, std::vector<cv::KeyPoint> vCurrentKeys)
{
     ...
     mpCamera2(nullptr), mTimeStereoMatch(0), mTimeORB_Ext(0)
{
    ...
    // std::cout << "Time extract ORB: " << mTimeORB_Ext << " (ms)" << std::endl;
#ifdef SAVE_TIMES
    ...
    if(mvKeys.empty())
        return;
    // cout << "N = " << N << endl;
    // cv::Mat temp = visualize(imGray, mvKeys);
    // cv::imshow("Image Extractor", temp); cv::waitKey(1);
